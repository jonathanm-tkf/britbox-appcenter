    ...
    return this.each(function() {
      ...
      that.updateSC = function(resize, next) {
        ...
        if (!resize) {
          ...
          if (!that.sc.suggestionHeight)
            that.sc.suggestionHeight = $('.autocomplete-suggestion', that.sc)
              .first()
              .outerHeight();
          if (that.sc.suggestionHeight)
      ...
      function suggest(data) {
        ...
        if (data.length && val.length >= o.minChars) {
          ...
          $(that.sc)
            .children()
            .wrapAll("<div class='scroll-custom' id='autocomplete-scroll'></div>");

          var acOptions = {alwaysShowTracks:true};
          window.libs.Scrollbar.init(document.getElementById('autocomplete-scroll'), acOptions);
      ...
      that.on('keydown.autocomplete', function(e) {
        ...
        if ((e.which == 40 || e.which == 38) && that.sc.html()) {
          ...
          const suggestions = $('.autocomplete-suggestion.selected');

          if (suggestions.length > 0) {
            const scroll = window.libs.Scrollbar.get(
              document.getElementById('autocomplete-scroll')
            );

            scroll.scrollTop += e.which === 40 ? 60 : -60;
          }
          return false;
        }
        // esc
      ...
      that.on('keyup.autocomplete', function(e) {
        if (!~$.inArray(e.which, [27, 35, 36, 37, 38, 39, 40])) {
          var val = that.val();
