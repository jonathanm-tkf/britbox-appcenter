...
import { BritboxAccountApi, BritboxContentApi, refreshToken } from './lib/sdk';
import Pinlogin from './lib/pinlogin';
...
function parentalModal() {
  const loginpin = new Pinlogin(document.querySelector('#loginpin'), {
    fields: 4,
    reset: true,
    hideinput: true,
    pattern: '^[0-9]$',
...
$(() => {
  ...
  $(document).on('click', '.subtitles-button button', e => {
    $(e.currentTarget)
      .closest('.subtitles-button')
      .addClass('active');
  ...
  $(document).on('click', '.subtitles-button ~ .content-subtitles a', e => {
    e.preventDefault();
